...
unpinned=$(grep -E "^\s+image:" docker-compose.yml | grep -vE "image:.*:[^:]+$" | grep -vE "image:.*@sha256:" | grep -vE "image:.*RELEASE\." || true)
if [ -n "$unpinned" ]; then
...
if [ "$newer_versions_found" = true ]; then
  echo ""
  echo "Error: Newer image versions are available. Please update docker-compose.yml to use the latest versions."
  exit 1
fi
